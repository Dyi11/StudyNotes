...
**傅里叶变换的窗口大小（点数）** 越大，频率分辨率越高（能区分更接近的频率），但时间分辨率越低（对时间变化的捕捉越模糊），1024 是常用值。
n_fft=1024 意味着：每个窗口截取 1024 个连续的采样点，对这 1024 个点做傅里叶变换。
结合采样率 sr 可以算出窗口的实际时间长度：窗口时长 = n_fft / sr（秒）
例如：若采样率 sr=22050 Hz（每秒 22050 个采样点），则 1024 个采样点对应的时间是 1024/22050 ≈ 0.046 秒（约 46 毫秒）

**窗口移动的步长（点数）** 小于窗口大小，确保相邻窗口有重叠，避免频谱跳变。256 意味着每次窗口移动 256 个采样点，重叠部分为 1024-256=768 点

当 n_fft=1024、sr=22050 时，频率分辨率 = 22050 / 1024 ≈ 21.5 Hz，即能区分开相差 ≥21.5 Hz 的两个频率（比如 100 Hz 和 122 Hz 可以区分，100 Hz 和 110 Hz 可能混在一起）。
若 n_fft=2048（窗口变大），频率分辨率 = 22050 / 2048 ≈ 10.7 Hz，能区分更接近的频率（比如 100 Hz 和 111 Hz 可以区分）。
简单说：窗口越大，频率轴上的 “刻度” 越密，能看清更细微的频率差异
...
#### （1）丢失“时间信息”
FT的结果只告诉我们“这段信号里包含哪些频率”，但不知道“这些频率分别在哪个时间出现”。
比如对一段“先有贝斯声、后有小提琴声”的音乐做FT，得到的频域图会同时显示贝斯的低频和小提琴的高频，但无法区分“低频在前、高频在后”，只能看作是“整个时间段内低频和高频都存在”。
